...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
#if 0 //probeersel dynamische pointer arrays
#define AANTAL_USARTS 2
volatile uint16_t *rx_back_p[AANTAL_USARTS], *rx_front_p[AANTAL_USARTS];
#else
volatile uint16_t *rx_back_p, *rx_front_p;
#endif
const uint8_t rx_p_array_size = 10;	//dit is geen magic number, je kan dit zien als een #define
volatile uint16_t *rx_p_array;
...
void USART_SendString(USART_TypeDef* USARTx, char *p)
{
	while(*p)
  {
    if(*p == '\n')
    {
      ...
      USART_SendData(USARTx, (uint16_t) '\r');
    }
    
		while(USART_GetFlagStatus(USARTx, USART_FLAG_TXE) == RESET){;}
    USART_SendData(USARTx, (uint16_t) *p++);
  }
}
...
/**
  ...
  */
uint16_t USART_ReceiveData(USART_TypeDef* USARTx) /* circular buffer implementation */
{
	//TODO functie werkt nu alleen wanneer er 1 USART actief is
	uint16_t c;
	...
	c = *rx_front_p;		//get data from buffer
	*rx_front_p = 0;		//clear buffer position
	
	//is het einde van de buffer bereikt?
	if (rx_front_p == &rx_p_array[rx_p_array_size-1]) {
		rx_front_p = rx_p_array;	//ja, terug naar begin van buffer
	} else {
		rx_front_p++;							//nee, volgende index
	}
...
void USART_ReceiveString(USART_TypeDef* USARTx, char *p)
{
	int i=0;
	
	do {
		while(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == RESET){;}
		p[i] = (char) USART_ReceiveData(USARTx);
		i++;
	} while ((p[i-1] != '\r') && (p[i-1] != '\n') && (p[i-1] != 0));
	...
	//TODO check of er nog ruimte vrij is in de char* buffer? 
}
...
void USART_ITConfig(USART_TypeDef* USARTx, uint32_t USART_IT, FunctionalState NewState)
{
  ...
  else
  {
    *(__IO uint32_t*)usartxbase &= ~itmask;
	...
	//TODO onderstaande code werkt alleen wanneer er 1 USART actief is
	//allocate memory for pointer array
	rx_p_array = (uint16_t *)malloc(sizeof(uint16_t)*rx_p_array_size);
	...
	//use [] notation to access array location
	//(THIS IS THE PREFERED WAY TO DO IT)
	for(i=0; i < rx_p_array_size; i++) {
		rx_p_array[i] = 0;	//of '\0', maakt in principe niks uit
	}
	
	//start pointers op index 0
	rx_back_p = rx_p_array;
	rx_front_p = rx_p_array;
	...
	//allocate memory for pointer array
	*temp = (uint16_t*)&rx_p_array[usart_nr];
	*temp = (uint16_t *)malloc(sizeof(uint16_t)*rx_p_array_size);
	...
	//(THIS IS THE PREFERED WAY TO DO IT)
	for(i=0; i < rx_p_array_size; i++) {
		rx_p_array[i+(rx_p_array_size*usart_nr)] = 0;	//of '\0', maakt in principe niks uit
	}
	
	//start pointers op index 0
	*temp = (uint16_t*)&rx_back_p;
	*temp += usart_nr;
	**temp = rx_p_array[rx_p_array_size*usart_nr];
	*temp = (uint16_t*)&rx_front_p;
	*temp += usart_nr;
	**temp = rx_p_array[rx_p_array_size*usart_nr];
#endif
}
